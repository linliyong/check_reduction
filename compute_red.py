...
print('This program is created by Liyong Lin. It is developed based on the two papers')
print('1 "reduction of distributions: definitions, properties and applications"')
print('2 "automatic generation of optimal reductions of distributions".\n')
print('A reduction of a distribution is a collection of smaller distributions that is')
print('equivalent to the source distribution w.r.t. the property of decomposability.')
print('Instead of verifying decomposability on the source distribution, we can now verify')
print('the decomposability on the collection of smaller distributions to reduce complexity.')
print('This program allows us to check for the existence of reductions, verify a candidate')
print('reduction, compute optimal reductions. For more details, please check the two papers\n')

print('1) A distribution is of the format')
print("                              ['ab','bc','cd','de']")
print('For any distribution "distr", we can use the command "existsred(distr)" to check')
print('for the existence of reductions. A reduction is of the format')
print("                       [['ab','bcd','de'],['ab','bc','cde']]")
print("For example, type 'existsred(['ab','bc','cd','de'])' to check whether ['ab','bc','cd','de']")
print('has a reduction. \n')
print('2) For any distribution "distr" and any candidate reduction "red", we can check')
print('whether "red" is a reduction of "distr" using "check_reduction(distr,red)"')
print("For example, type 'check_reduction(['ab','bc','cd','de'],[['ab','bcd','de'],['ab','bc','cde']])'")
print("to check whether [['ab','bcd','de'],['ab','bc','cde']] is a reduction of ['ab','bc','cd','de'].\n")
print('Have fun!')
print('------------------------------------------------------------------------------')

#check whether is a distribution, give warning.
def check_distrib(distr):
    distr_prim=distr
    if len(distr)==len(sim_distr(distr_prim)):
        return 1
    else:
        return 0
...
def existsred(distr):
    if not(check_distrib(distr)):
        print('warning, the input is not a distribution')
        return
    sa=mini_merge(distr)
...
#L_{cand} based refutation 2
def dependence_refute2(distr,distrset):
    s=''
    for i in range(0, len(distr)):
        s=s+distr[i]
    ...
    for i in range(0, len(distrset)):
        if not(dependence_decom2(distr,nfun,distrset[i],alpha)):
            return 0
    return 1
 
#check decomposability of L_{cand} with respect to distr1 2            
def dependence_decom2(distr,nfun,distr1,alpha):
    ...
    for i in range(0, len(dif)):
        ...
        if check_cover2(nfun,distr1,bd, dif[i],alpha, graph_distr1,distr):
            return 1
    return 0

def check_cover2(nfun,distr1,bd, sub,alpha, graph_distr1,distr):
    sub=list(sub)
    ...
    for i in range(0, len(ext)):
        if not(check_sat[ext[i]]==set(range(0,len(distr)))):
            determ=determ #addd more#
            #add more here to generalize the proof...
        else:
            determ=determ+[ext[i]]
    return 1
